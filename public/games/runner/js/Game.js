...
Game.prototype = {
    ...
    _initScene : function() {
        ...
        var camera = new BABYLON.ArcRotateCamera("", -Math.PI/2, Math.PI/3, 20, new BABYLON.Vector3(0,0,6), scene);
    ...
    run : function() {
        ...
        var bushMat = new BABYLON.StandardMaterial("bushMat",this.scene);
        bushMat.diffuseTexture = new BABYLON.Texture("./assets/bush/bush.png",this.scene);
        bushMat.diffuseTexture.hasAlpha = true;
        bushMat.backFaceCulling = false;
        var bush = BABYLON.MeshBuilder.CreatePlane("bush",{width:4,height:4},this.scene);
        bush.material = bushMat;
        _this.assets["bush"] = {meshes: bush};

        var rabbitMat = new BABYLON.StandardMaterial("rabbitMat",this.scene);
        rabbitMat.diffuseTexture = new BABYLON.Texture("./assets/rabbit/rabbit.png",this.scene);
        rabbitMat.diffuseTexture.hasAlpha = true;
        rabbitMat.backFaceCulling = false;
        var rabbit = BABYLON.MeshBuilder.CreatePlane("rabbit",{width:1,height:2},this.scene);
        rabbit.material = rabbitMat;
        _this.assets["rabbit"] = {meshes: rabbit};

        var woodpileMat = new BABYLON.StandardMaterial("woodpileMat",this.scene);
        woodpileMat.diffuseTexture = new BABYLON.Texture("./assets/woodpile/woodpile.png",this.scene);
        woodpileMat.diffuseTexture.hasAlpha = true;
        woodpileMat.backFaceCulling = false;
        var woodpile = BABYLON.MeshBuilder.CreatePlane("woodpile",{width:4,height:2},this.scene);
        woodpile.material = woodpileMat;
        _this.assets["woodpile"] = {meshes: woodpile};

        var stonepileMat = new BABYLON.StandardMaterial("stonepileMat",this.scene);
        stonepileMat.diffuseTexture = new BABYLON.Texture("./assets/stonepile/stonepile.png",this.scene);
        stonepileMat.diffuseTexture.hasAlpha = true;
        stonepileMat.backFaceCulling = false;
        var stonepile = BABYLON.MeshBuilder.CreatePlane("stonepile",{width:2,height:2},this.scene);
        stonepile.material = stonepileMat;
        _this.assets["stonepile"] = {meshes: stonepile};

        var abrahamMat = new BABYLON.StandardMaterial("abrahamMat",this.scene);
        abrahamMat.diffuseTexture = new BABYLON.Texture("./assets/abraham/abraham.png",this.scene);
        abrahamMat.diffuseTexture.hasAlpha = true;
        abrahamMat.backFaceCulling = false;
        var abraham = BABYLON.MeshBuilder.CreatePlane("abraham",{width:2,height:4},this.scene);
        abraham.material = abrahamMat;
        _this.assets["abraham"] = {meshes: abraham};

        var isaacMat = new BABYLON.StandardMaterial("isaacMat",this.scene);
        isaacMat.diffuseTexture = new BABYLON.Texture("./assets/isaac/isaac.png",this.scene);
        isaacMat.diffuseTexture.hasAlpha = true;
        isaacMat.backFaceCulling = false;
        var isaac = BABYLON.MeshBuilder.CreatePlane("isaac",{width:2,height:4},this.scene);
        isaac.material = isaacMat;
        _this.assets["isaac"] = {meshes: isaac};

        var ramMat = new BABYLON.StandardMaterial("ramMat",this.scene);
        ramMat.diffuseTexture = new BABYLON.Texture("./assets/ram/ram.png",this.scene);
        ramMat.diffuseTexture.hasAlpha = true;
        ramMat.backFaceCulling = false;
        var ram = BABYLON.MeshBuilder.CreatePlane("ram",{width:2,height:1},this.scene);
        ram.material = ramMat;
        _this.assets["ram"] = {meshes: ram};
    ...
    start : function() {
        ...
        setInterval(function(){
            _this.gamespeed+=0.01;
        },1000);

        // Start the game when space is pressed !
        $(window).keydown(function(evt) {
            this.console.log(evt.keyCode);
            $("#spaceToStart").html("Space to start!:"+evt.keyCode);
    ...
    _initGame : function() {
        ...
        this.scene.registerBeforeRender(function() {
            ...
            _this.light.position.z = _this.player.position.z - 5;
            _this.light.position.y = 20;
    ...
    _initParticles : function() {
        ...
        snow.particleTexture = new BABYLON.Texture("assets/dust.png", this.scene);
