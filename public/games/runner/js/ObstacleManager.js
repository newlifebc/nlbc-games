...
ObstacleManager.prototype.sendPattern = function(pat, zpos) {
    for (var p=0; p<pat.length; p++) {
        ...
        if (pat[p] == 1)  {
            ...
            var o = this.game.assets['stonepile'].meshes.createInstance("stonepile"+p);
        ...
        if (pat[p] == 2) {
            ...
            var b = this.game.assets['woodpile'].meshes.createInstance("woodpile"+p);
        ...
        if (pat[p] == 3) {
            ...
            var s = this.game.assets['rabbit'].meshes.clone("rabbit"+p);
            s.setEnabled(true);
